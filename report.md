...
有。2026 年 5 月我与好友（也是队友）两人协作开发了一个教学平台（仓库 `hk-iflytek-4/design-cnn-watermark-detection`），从建仓当天即参与，历时约两周。项目是面向 CNN 水印检测课程的教学系统，包含学生端演示（Vue 3 + FastAPI + JupyterHub）和教师端平台（实验编辑器 + 班级管理），全程使用 Git 协作，两人累计提交 **285 次**。
